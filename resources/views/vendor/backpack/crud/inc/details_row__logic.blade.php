...
<script>
  if (typeof registerDetailsRowButtonAction != 'function') {
    function registerDetailsRowButtonAction() {
          ...
          $('#crudTable_{!! $crud->cruds_name_array !!} tbody').off('click', 'td .details-row-button');

          // Make sure the ajaxDatatables rows also have the correct classes
          $('#crudTable_{!! $crud->cruds_name_array !!} tbody td .details-row-button').parent('td')
          ...
          $('#crudTable_{!! $crud->cruds_name_array !!} tbody td .details-control').on('click', function (e) {
            e.stopPropagation();
  ...
  crud_{!! $crud->cruds_name_array !!}.addFunctionToDataTablesDrawEventQueue('registerDetailsRowButtonAction');
</script>
@endif