@php
    ...
    if($private && isset($field['value']) && $field['value'] != "")
    {
        $path = $field['value'];
        $exists = Storage::disk($field['disk'])->exists($path);
        if($exists)
        {            
            $full_path = Storage::disk($field['disk'])->path($path);
            $base64 = base64_encode(Storage::disk($field['disk'])->get($path));
